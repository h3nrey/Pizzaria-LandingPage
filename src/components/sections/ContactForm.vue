    ...
    <form ref="form" @submit.prevent="sendEmail" class="flex flex-col gap-6">
      <fieldset class="flex">
        ...
        <div class=" grid grid-cols-2 gap-4">
          ...
          <input name="from_name" type="text" id="firstname" class=" bg-gray visited:outline-darkGray rounded-sm p-[0.7rem]" placeholder="first">
      ...
      <div class="flex flex-col gap-6">
        <div class="flex flex-col text-darkGray font-medium">
          ...
          <input name="reply_to" type="email" id="email" class=" bg-gray visited:outline-darkGray rounded-sm p-[0.7rem]" placeholder="Email Address">
        </div>

        <div class="flex flex-col text-darkGray font-medium">
          <label for="phone" class=" capitalize">phone</label>
          <input name="phone" type="text" id="phone" class=" bg-gray visited:outline-darkGray rounded-sm p-[0.7rem]" placeholder="Phone Number">
        </div>
      </div>
      ...
      <div class="flex flex-col gap-6">
        <div class="flex flex-col text-darkGray font-medium">
          ...
          <input name="subject" type="subject" id="subject" class=" bg-gray visited:outline-darkGray rounded-sm p-[0.7rem]" placeholder="Subject">
        </div>
        ...
        <div class="flex flex-col">
          ...
          <div class="relative z-0 w-full cursor-pointer">
            <span class="absolute z-10 top-1/2 left-[100%] -translate-y-1/2 -translate-x-[100%] p-4 height-full w-[10%] outline-1 outline-darkGray pointer-events-none">
              <CaretIcon class=" text-black" :size="20"/>
            </span>
            <select 
            name="restaurant"
          v-model="selected" 
          id="location" 
          class="bg-gray text-darkGray visited:outline-darkGray rounded-sm p-[0.7rem] appearance-none w-full cursor-pointer" 
          >
            <option disabled value="default" class="">Select a Restaurant</option>
            <option v-for="(option, index) in options" class=" capitalize" :value="option.value" :key="index">{{option.text}}</option>
            </select>
          </div>
        ...
        <div class="flex flex-col text-darkGray font-medium">
          ...
          <textarea name="feedback" id="comment" cols="30" rows="5" class="bg-gray visited:outline-darkGray rounded-sm p-[0.7rem]"></textarea>
        </div>
